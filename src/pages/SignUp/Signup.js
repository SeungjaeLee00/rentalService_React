...
function Signup() {
	...
	const SendEmail = () => {
		...
		if (email.current.value) {
			...
			axios
				.post("/api/email/sign-up?email=" + email.current.value)
				.then((response) => {
					setEmailCheck(true);
				...
				<form onSubmit={handleSubmit(onSubmit)}>
					<div className="email-wrap">
						<div className="emailTop">
							<label>이메일</label>
							<input className="inputField" ref={email} placeholder="abcdef@google.com" />
						</div>
							...
							<input
								...
								{...register("password", {
									...
									pattern: {
										...
										message: "비밀번호는 숫자, 영문, 특수문자를 포함한 8글자 이상이어야 합니다.",
									},
								})}
							/>
						</div>
