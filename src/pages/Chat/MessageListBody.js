	...
	return (
		<div className="message-content">
			<table>
				<thead>
					<tr>
						<th className="sender" key={2}>
							보낸이
						</th>
						<th key={3}>게시물</th>
						<th key={4} className="th3">
							내용
						</th>
						<th className="date" key={5}>
							날짜
						</th>
						<th className="check" key={6}>
							읽음상태
						</th>
					</tr>
				</thead>
					...
					{message.map((a) => (
						...
						>
							...
							<td className="check-body">
								{a.checked ? (
									<div style={{ color: "gray" }}>읽음</div>
								) : (
									<div style={{ fontWeight: "bold" }}>읽지않음</div>
								)}
							</td>
						</tr>
					))}
