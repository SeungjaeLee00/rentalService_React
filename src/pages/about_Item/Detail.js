				...
				<div className="Detail_Item_wrap">
					<div className="Detail_Item_Img">
						...
						<OneItem
							item={item}
							id={id}
							fetchPostInfo={fetchPostInfo}
							location={location}
							setItem={setItem}
							navigate={navigate}
							openReportModal={openReportModal}
							showReportPopup={showReportPopup}
							closeReportnModal={closeReportnModal}
...
function OneItem(props) {
	function LikeAdd() {
		axios
			.post("/api/posts/" + props.id + "/likes", null, {
		...
		<div>
			<div className="Detail_Item_wrap">
				...
				<div>
					<img
						className="oneImg"
						src={"https://sharingplatformbucket.s3.ap-northeast-2.amazonaws.com/post/" + props.item.imageName}
					/>
				</div>
				...
				<div className="Item_About">
					...
					<div className="Detail_Item_Name_Price">
						...
						<div className="Detail_Item_Price">
							{props.item.item.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}원
						</div>
					</div>
					<div style={{ marginTop: "15px" }}>
						...
						<div className="nickname-btn">
							<div className="profile" onClick={props.onProfileClick}>
								<AiOutlineUser />
								{props.item.writer.nickname}
							</div>
							<button onClick={props.openReportModal} variant="secondary">
								<AiOutlineAlert style={{ color: "red" }} />
							</button>
						</div>
					</div>
					<div className="Item_Button">
						<button className="likebtn" onClick={LikeAdd}>
							<span>
								<AiTwotoneHeart />
							</span>
							&nbsp;<span>{props.item.likes}</span>
						</button>
						<button className="sendbtn" onClick={() => props.navigate("/itemmain/detail/chat", { state: props.item })}>
							쪽지보내기
						</button>
