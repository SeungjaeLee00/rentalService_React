...
export default function Comment(props) {
	...
	const CreateComment = () => {
		...
		if (checkreply == true) {
			...
			console.log(data);
			axios
				.post(`/api/posts/${props.postid}/comments`, data, {
					headers: {
				...
				{comment.map((a, index) => {
					return (
						<div key={a.commentId} className="one-comment">
							<Div active={selectreply[index]}>
								<div className="onecomment-info">
									...
									<div className="onecomment-btn">
										...
										<button
											onClick={() => {
												CreateCommentBtn(a.commentId, index);
											}}
										>
											답글 달기
										</button>
							...
							<div className="reply">
								{openreply[index]
									? a.children.map((reply, replyindex) => {
											return (
												<div key={reply.commentId} className="reply-one-wrap">
													<div className="reply-info">
														<div>
															<div className={`comment-${nickname == reply.nickname ? "writername" : "name"}`}>
																{reply.nickname}
															</div>
