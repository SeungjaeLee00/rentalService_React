		...
		<div className="report-wrap">
			...
			<div className="report-info">
				...
				<div className="type-selector">
					<button className={isActive1 ? "all" : "buttonDefault"} onClick={() => handleTypeChange("ALL")}>
						모두
					</button>
					<button className={isActive2 ? "bug" : "buttonDefault"} onClick={() => handleTypeChange("BUG")}>
						버그
					</button>
					<button className={isActive3 ? "post_" : "buttonDefault"} onClick={() => handleTypeChange("POST_REPORT")}>
						게시물 신고
					</button>
				</div>
							...
							(report, index) =>
								...
								(selectedType === "ALL" || report.reportType === selectedType) && (
									<tr key={report.reportId}>
										...
										<td>
											<p className="report-info-item">{report.content}</p>
										</td>
										<td>
