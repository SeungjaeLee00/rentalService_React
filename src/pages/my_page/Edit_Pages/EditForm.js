...
const EditForm = (props) => {
	...
	const [address, setAddress] = useState(
		props.userData.address.city +
			" " +
			props.userData.address.district +
			" " +
			...
			<table className="my-datatable">
				<tbody className="my-datatbody">
					<tr>
						<td>이메일</td>
						<td>
							<input className="input-email" name="email" value={email} onChange={onChange}></input>
						</td>
					</tr>
					<tr>
						<td>닉네임</td>
					...
					<tr>
						<td>핸드폰 번호</td>
						<td>
							<input className="input-phone" name="phoneNumber" value={phonenumber} onChange={onChange}></input>
						</td>
					</tr>
					<tr>
						<td>주소</td>
					...
					{/* 주소찾기 클릭시 열리는모달 */}
					{addressisopen && (
						<Address addresstoggle={addresstoggle} zipcodehandle={zipcodehandle} addresshandle={addresshandle} />
					)}
					<tr>
						<td>소개</td>
						<td>
							<input className="input-introduce" name="introduce" value={introduce} onChange={onChange} />
						</td>
					</tr>
					<tr>
						<td>프로필사진</td>
						<td>
							<input type="file" name="image" onChange={saveFile}></input>
						</td>
					</tr>
				</tbody>
			</table>
			<button className="CompleteEditBtn" onClick={patchmyinfo}>
				수정완료
			</button>
		</div>
	);
