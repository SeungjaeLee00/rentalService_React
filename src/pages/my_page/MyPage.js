...
export default function MyPage() {
	...
	if (mypost.loading || myrent.loading || myborrow.loading || myreview.loading || mywriterw.loading)
		return <div>로딩중</div>;
	...
	return (
		<div>
			...
			<MyPageTop
				screen={screen}
				mypost={mypost.data.totalElements}
				myrent={myrent.data.totalElements}
				myborrow={myborrow.data.totalElements}
				myreview={myreview.data.totalElements}
				mywriterw={mywriterw.data.totalElements}
			/>

			<div className="mypagebottom">
				{/* 마이페이지 왼쪽 nav */}
				{screen >= 750 ? (
					<div className="bottom-leftnav">
						<Sidebar />
					</div>
				) : null}
